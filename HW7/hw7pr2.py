...
def find_cost(X, y, W , reg):
	...
	"*** YOUR CODE HERE ***"
	temp = X @ W - y
	err = float(temp.T @ temp)
	cost = (err + reg * np.abs(W).sum()) / len(y)
	"*** END YOUR CODE HERE ***"
	return cost
...
def grad_lasso(
	...
	while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
		...
		"*** YOUR CODE HERE ***"
		# Randomly select indices for entries into a batch using np.random.randint()
		i = np.random.randint(0, m, size=batch_size)
		# Find the gradient using the batch
		W_grad = find_grad(X[i], y[i], W, reg=reg)
		# Apply the threshold function prox(), according to the problem statement, to update W
		W = prox(W - lr * W_grad, reg * lr)
		# Update the cost and append it to obj_list
		cost = find_cost(X[i], y[i], W, reg=reg)
...
def lasso_path(X, y, tau_min=1e-8, tau_max=10, num_reg=10):
	...
	for index in range(num_reg):
		...
		# 		as they are.
		"""*** YOUR CODE HERE ***"""
		W[:, index] = grad_lasso(X, y, reg=reg, lr=1e-12, max_iter=1000, batch_size=1024, print_freq=1000)[0].flatten()
		"""*** END YOUR CODE HERE ***"""
