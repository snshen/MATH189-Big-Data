...
def linreg(X, y, reg=0.0):
	...
	I = np.eye(X.shape[1])
	I[0, 0] = 0
	W_opt = np.linalg.solve(X.T@X + I*reg, X.T@y)
	"*** END YOUR CODE HERE ***"
	return W_opt
...
def find_RMSE(W, X, y):
	...
	"*** YOUR CODE HERE ***"
	y_pred = predict(W, X)
	RMSE = np.sqrt(np.linalg.norm(y - y_pred, 2) ** 2 / X.shape[0])
	"*** END YOUR CODE HERE ***"
	return RMSE
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	reg_list.sort()
	W_list = [linreg(X_train, y_train, reg = parameter) for parameter in reg_list]
...
def norm_vs_lambda(X_train, y_train, X_val, y_val):
	...
	reg_list.sort()
	W_list = [linreg(X_train, y_train, reg = parameter) for parameter in reg_list]
	norm_list = [np.linalg.norm(weight, 2) for weight in W_list]
	"*** END YOUR CODE HERE ***"
...
def linreg_no_bias(X, y, reg=0.0):
	...
	m = X.shape[0]
	ones = np.eye(m)
	mat = X.T @ (np.eye(m) - np.ones(m) / m)
	W_opt = np.linalg.solve(mat @ X + reg * np.eye(mat.shape[0]), mat @ y)
