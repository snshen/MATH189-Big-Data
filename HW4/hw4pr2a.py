...
def NLL(X, y, W, reg=0.0):
	...
	"""
	"*** YOUR CODE HERE ***"
	temp = np.multiply(y, np.log(sigmoid(X @ W)))+ np.multiply((1-y),np.log(1- igmoid(X @ W)))
	summation = -sum(temp)+reg/2*np.linalg.norm(W)**2
	nll = summation.item(0)
	"*** END YOUR CODE HERE ***"
	return nll
...
\
	...
	while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
		...
		"*** YOUR CODE HERE ***"
		val = NLL(X, y, W, reg = reg)
		if np.isnan(val):
			break
		nll_list.append(val)
		W -= lr * grad_logreg(X, y, W, reg = reg)
		"*** END YOUR CODE HERE ***"
...
def newton_step(X, y, W, reg=0.0):
	...
	diag = np.diag(np.squeeze(np.asarray(np.multiply(sigmoid(X @ W), 1. - sigmoid(X @ W)))))
...
def get_description(X, y, W):
	...
	for index in range(X.shape[0]):
		...
		if pred == 1:
			...
			if actual == 1:
				prob_p += 1
	accuracy = 1* prob_a / X.shape[0]
	precision = 1* prob_p / total_p
	recall = 1 * prob_r / total_r
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	"*** YOUR CODE HERE ***"
	reg_list = [0, 0.01, 0.05, 0.1, 0.5, 1.0, 5.0, 10.0, 20.0, 30.0]
	reg_list.sort()
	for val in reg_list:
		reg = val
		W_opt,_ = grad_descent(X_train, y_train, reg = reg, lr = 2e-4, print_freq = 100)
