...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	e_mu = np.exp(X @ W)
	prob = e_mu / e_mu.sum(axis=1).reshape(-1, 1)
	truth = y * np.log(prob)
	Nll = -truth.sum(axis=1).sum() + reg * np.diag(W.T @ W).sum()
	"*** END YOUR CODE HERE ***"
	return NLL
...
def accuracy_vs_lambda(X_train, y_train_OH, X_test, y_test, lambda_list):
	...
	for reg in lambda_list:
		...
		"*** YOUR CODE HERE ***"
		W, _ = grad_descent(X_train, y_train_OH, reg=reg, lr=2e-5, print_freq=50)
		y_pred = predict(X_test, W)
		accu_list.append(get_accuracy(y_pred, y_test))
		"*** END YOUR CODE HERE ***"
