...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
	...
	while iter_num < max_iter:
		...
		for i in range(m):
			diff = X[i, :] - clusters
			label[i] = np.argsort(np.linalg.norm(diff, axis=1)).item(0)
		"*** END YOUR CODE HERE ***"
		...
		for j in range(k):
			...
			if len(i) > 0:
				clusters[j, :] = X[i].mean(axis=0)
		cost_list.append(k_means_cost(X, clusters, label))
		"*** END YOUR CODE HERE ***"
...
if __name__ == '__main__':
	...
	"*** YOUR CODE HERE ***"
	clusters, label,_ = k_means(X, opt_k)
